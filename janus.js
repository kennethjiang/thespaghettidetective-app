...
export function Janus(gatewayCallbacks) {
    ...
    function handleEvent(json) {
        ...
        if (json["janus"] === "keepalive") {
            ...
            Janus.warn("Unknown message '" + json + "'");
        }
    }
    ...
    async function streamsDone(handleId, jsep, media, callbacks, stream) {
        ...
        if (jsep === null || jsep === undefined) {
            ...
            try {
                await config.pc.setRemoteDescription(new RTCSessionDescription(jsep));
                Janus.log("Remote description accepted!");
                await createAnswer(handleId, media, callbacks);
            } catch(e) {
                callbacks.error(e);
            }
        }
    ...
    async function prepareWebrtc(handleId, callbacks) {
        ...
        if (config.pc !== undefined && config.pc !== null) {
            console.log('[PC CONFIG NOT NULL]')
    ...
    async function prepareWebrtcPeer(handleId, callbacks) {
        callbacks = callbacks || {};
        callbacks.success = (typeof callbacks.success == "function") ? callbacks.success : Janus.noop;
        callbacks.error = (typeof callbacks.error == "function") ? callbacks.error : webrtcError;
        var jsep = callbacks.jsep;
    ...
    async function createOffer(handleId, media, callbacks) {
        ...
        try {
            var offer = await config.pc.createOffer();
    ...
    async function createAnswer(handleId, media, callbacks) {
        callbacks = callbacks || {};
        callbacks.success = (typeof callbacks.success == "function") ? callbacks.success : Janus.noop;
        callbacks.error = (typeof callbacks.error == "function") ? callbacks.error : Janus.noop;
        var pluginHandle = pluginHandles[handleId];
        ...
        Janus.debug(mediaConstraints);
        try {
            var answer = await config.pc.createAnswer(mediaConstraints);
